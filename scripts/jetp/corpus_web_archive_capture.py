...
3. otherwise Save Page Now is asked, anonymously, for a capture, and the
   job is followed to its end: ``captured`` with the snapshot's address and
   timestamp, or ``failed`` with the reason.
...
class Wayback:
    ...
    def _request(self, method, url, **kwargs):
        ...
        for delay in (*self.backoff, None):
            ...
            self.sleep(max(delay, int(wait)) if wait.isdigit() else delay)
        return response
    ...
    def save(self, url):
        ...
        response = self._request('POST', SAVE, data={'url': url},
    ...
    def _follow(self, job):
        ...
        while waited <= self.poll_limit:
            ...
            response = self._request('GET', STATUS + job, headers={'Accept': 'application/json'})
            if response.status_code != 200:
                continue
...
def capture_one(document, wayback, reuse_window_days, save=True):
    ...
    if not save:
        return dict(row, outcome='failed', error='wayback_unreachable' + lookup)
    try:
...
def run(documents, output, wayback, reuse_window_days=365, breaker=5, limit=None):
    ...
    log.info('%d collected documents, %d to capture', len(documents), len(pending))
    unreachable = 0
    for n, document in enumerate(pending, 1):
        key = (document['source_id'], document['url'])
        try:
            row = capture_one(document, wayback, reuse_window_days, save=unreachable < breaker)
        ...
        log.info('[%d/%d] %s: %s %s', n, len(pending), key[0], row['outcome'], row['error'])
    return rows
...
def main(argv=None):
    ...
    parser.add_argument('--pace', type=float, default=15.0,
                        help='seconds between Save Page Now requests')
    ...
    rows = run(documents, args.output, Wayback(pace=args.pace),
