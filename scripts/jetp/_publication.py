...
def publish(release_id, displays, combinations):
    ...
    financial claims are deliberately unavailable until every named contributor
    has accepted reconciled ownership.
    """
    ...
    return {'format_version': FORMAT_VERSION, 'release_id': release_id,
            'displays': sorted(normalized, key=lambda row: row['display_id']),
            'combinations': sorted(selected, key=lambda row: (row['subject'], row['measure'], row['perimeter'])),
...
def reported_position_sidecar(config, countries):
    ...
    for code, country in sorted(countries.items()):
        ...
        for route, display_id in ((f'#overview', f'overview-{code}-headline'),
