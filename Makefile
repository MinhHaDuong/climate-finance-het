...
# Zoo figures (ZOO_SCHEMATICS / ZOO_RESULT_FIGS, defined in paths.mk, and so
...
# Reranker human-validation evidence (ticket 0372): rebuilt from the two
# surviving artifacts — the stratified sample sheet (git-tracked in docs/)
# and the per-quintile rates recorded in the script itself with provenance.
# No corpus data needed.
deliverables/_shared/tables/tab_reranker_validation.csv: scripts/figures/export_reranker_validation.py scripts/utils.py docs/reranker_hitl_stratified.csv
	$(PYTHON) $< --output $@

# Removal ablation per stratum (ticket 0337): what the six-flag filter
# removes, by source / language / period / DOI / citation decile. Reads the
# pre-removal catalog — computing from refined_works would be circular.
deliverables/_shared/tables/tab_filter_ablation.csv: scripts/figures/export_filter_ablation.py scripts/utils.py data/catalogs/extended_works.csv
	$(PYTHON) $< --output $@
...
corpus-tables: deliverables/_shared/tables/tab_corpus_sources.csv deliverables/_shared/tables/tab_corpus_sources.md \
               ...
               deliverables/_shared/tables/tab_reranker_validation.csv \
               deliverables/_shared/tables/tab_filter_ablation.csv
