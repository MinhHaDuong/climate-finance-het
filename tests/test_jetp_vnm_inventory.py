...
def test_wrapped_count_is_not_an_ordinal_or_named_asset():
    rows = assemble_inventory([
        {'annex': 'I.1', 'page': 157, 'cells': ['36', 'Convert and replace 244 locomotives and\n80 generator cars to use electricity and\ngreen energy', 'National'], 'source_wording': '36 Convert and replace 244 locomotives and\n80 generator cars to use electricity and\ngreen energy'},
        {'annex': 'I.2', 'page': 159, 'cells': ['1', 'Pilot testing for 3-4 suitable coal-fired power plants (CFPPs)', 'TBD'], 'source_wording': '1 Pilot testing for 3-4 suitable coal-fired power plants (CFPPs) TBD'},
...
def test_continuation_preserves_all_cells_and_both_page_locators():
    rows = assemble_inventory([
        {'annex': 'I.2', 'page': 161, 'cells': ['23', 'Unreviewed proposal', 'first'], 'source_wording': '23 Unreviewed proposal first'},
        {'annex': 'I.2', 'page': 162, 'cells': ['', 'continued wording', 'second'], 'source_wording': 'continued wording second'},
    ], require_complete=False)
    assert len(rows) == 1
    assert rows[0]['pages'] == [161, 162]
    assert rows[0]['source_cells'] == ['23', 'Unreviewed proposal\ncontinued wording', 'first\nsecond']
    assert rows[0]['source_wording'] == '23 Unreviewed proposal first\n\f\ncontinued wording second'
...
def test_saved_rmp_has_all_279_rows_without_solar_hydro_identity_join():
    ...
    if not source:
        pytest.skip('Set VNM_INVENTORY_PDF to the saved English RMP for source-byte audit')
