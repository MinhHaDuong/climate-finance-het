...
def test_zaf_candidate_reconciles_inventory_legacy_and_current_views(tmp_path):
    ...
    # acquisition rows; 1120 added the reviewed AfDB MURP project row.
    # 0878 retires this legacy sidecar.
    assert len(result['legacy_dispositions']) == 831
    ...
    assert len(result['comparison']['existing_project_ids']) == 5
    assert 'project-zaf-murp' in result['comparison']['existing_project_ids']
