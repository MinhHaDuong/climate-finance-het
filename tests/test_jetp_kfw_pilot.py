...
def test_identical_starts_do_not_validate_zero_delay():
    node = ET.fromstring('<iati-activity><activity-date type="1" iso-date="2016-01-01"/>'
...
def test_sample_round_robin_excludes_challenges_and_retains_all_candidates():
    units = [{'unit_id': str(i), 'country': 'MA', 'sector': sector, 'instrument': '110'}
             for sector, i in [('energy', 1), ('energy', 2), ('non-energy', 3),
                               ('non-energy', 4), ('non-energy', 5)]]
    selected = pilot.select_sample(units, {'1'}, size=3)
    assert [r['unit_id'] for r in selected if r['selected'] == 'true'] == ['2', '3', '4']
