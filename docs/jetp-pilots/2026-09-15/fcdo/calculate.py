...
def clocks(record, parent=None):
    ...
    result = {
        "actual_start": "",
        "earliest_observed_expenditure": "",
        "earliest_observed_transfer": "",
        "first_spending": "",
    ...
    for code, key in [
        ("4", "earliest_observed_expenditure"),
        ("3", "earliest_observed_transfer"),
    ]:
        observed = [
            t["transaction-date"]["iso-date"]
...
def selection(records):
    ...
    for d in records:
        if (
            d["hierarchy"] == 1
            and country(d) in COUNTRIES
            and d["iati_identifier"] not in challenge
        ):
            key = (
                country(d),
                sector(d),
                str(d.get("default_finance_type_code", "missing")),
            )
            strata[key].append(d)
    ordered = []
...
def run(archive, output, new_archive=None, assessment_path=None, evidence_path=None):
    ...
    assessments = (
        list(csv.DictReader(assessment_path.open())) if assessment_path else []
    )
    detailed = {}
    sources = {}
    ...
    evidence.append(
        dict(
            evidence_id="catalogue",
            claim="complete acquired response; not historical census",
            issuer="FCDO",
            url="https://fcdo.iati.cloud/search/activity/",
            title_date="GB-GOV-1 catalogue; acquired 2026-09-14",
            page_section="response.docs",
            ...
            sha256=sha(catalogue),
            limitations="limited fl projection; sector vocabulary and finance code omitted",
        )
    )
    for ident, path in sorted(sources.items()):
        bundle = (
            "0735-round3/us-uk/" + path.name
            if path.parent == raw
            else "0739-pilot/" + path.name
        )
        evidence.append(
            dict(
                evidence_id=ident,
                claim="structured activity dates, relations and documentary pointers",
                issuer="FCDO",
                url="https://fcdo.iati.cloud/search/activity/?fl=*&format=json&q=iati_identifier:"
                + ident,
                title_date=ident + "; acquired 2026-09-14",
                page_section="response.docs[0].json.*",
                ...
                archive_path="data/jetp/audit-evidence/" + bundle,
                sha256=sha(path),
    ...
    for d in sorted(records, key=lambda d: d["iati_identifier"]):
        ...
        parents = [
            r["ref"]
            for r in structured(detail, "related-activity")
            if str(r.get("type")) == "1"
        ]
        units.append(
            dict(
                ...
                source_unit="programme" if d["hierarchy"] == 1 else "component",
                country=country(d),
                country_raw="|".join(d.get("recipient_country_code", [])),
                related_ids_raw="|".join(d.get("related_activity_ref", [])),
                sector_raw="|".join(d.get("sector_code", [])),
                sector_mapped=sector(detail),
                instrument_raw=str(detail.get("default_finance_type_code", "")),
                instrument_mapped="unknown",
                snapshot=SNAPSHOT,
                observation_status="observed_current_snapshot",
                raw_status=d.get("activity_status_code", ""),
        ...
        for number, transaction in enumerate(structured(detail, "transaction")):
            events.append(
                event(
                    ident,
                    "transaction_type_" + str(transaction["transaction-type"]["code"]),
                    "json.transaction[" + str(number) + "]",
                    transaction,
                    transaction.get("transaction-date", {}).get("iso-date", ""),
                    "reported date; aggregated period where described",
                    ident,
            ...
            for stage in ("approval", "signature", "procurement", "first_spending"):
                events.append(
                    event(
                        ...
                        "unvalidated",
                        "no independently validated stage document / earlier coverage",
                    )
                )
    ...
    write_csv(output / "coverage.csv", build_coverage(units, detailed))
    corroborated = {
        a["case_id"]
    ...
    write_csv(
        output / "document-coverage.csv",
        build_document_coverage(selected, detailed, assessments),
    )
    profile = dict(
        records=len(records),
        hierarchy=dict(Counter(d["hierarchy"] for d in records)),
        status_by_hierarchy={
            str(h): dict(
                Counter(
                    d.get("activity_status_code")
                    for d in records
                    if d["hierarchy"] == h
                )
            )
        ...
        sample_candidates=sum(r["role"] == "sample" for r in selected),
        sample_selected=sum(
            r["role"] == "sample" and r["selected"] == "true" for r in selected
        ),
        challenge_programmes=sum(
            r["role"] == "challenge" and r["source_unit"] == "programme"
            for r in selected
        ),
        challenge_components=1,
        detailed_records=len(detailed),
        frozen_sector="mixed-or-unmapped: catalogue omitted vocabulary",
    ...
    for (ctry, sec, instrument, source_unit), members in sorted(groups.items()):
        coverage.append(
            dict(
                ...
                entry_cohort="unknown_or_unvalidated",
                baseline="current_snapshot",
                stage="catalogue_presence",
                unit=source_unit,
                count_kind="observed",
                count=len(members),
                missing_count="",
                lost_visibility_count="",
                calendar_coverage=SNAPSHOT,
                denominator_rule="complete acquired export at hierarchy; no replication of multi-country records",
                evidence_ids="catalogue",
                reason="retention and historical loss unknown",
            )
        )
        if ctry not in COUNTRIES:
            continue
        for target, cutoffs in LANDMARKS.items():
            for label, cutoff in zip(
                ("early_calendar", "before_discussions", "before_declaration"),
                cutoffs,
                strict=True,
            ):
                for stage in (
                    ...
                    observed = (
                        [
                            clocks(detailed.get(u["unit_id"], {}))["actual_start"]
                            for u in members
                        ]
                        if stage == "actual_start"
                        else [""] * len(members)
                    ...
                    for kind in ("retrospective", "member", "pending"):
                        ...
                        coverage.append(
                            dict(
                                ...
                                sector=sec,
                                instrument=instrument or "missing",
                                entry_cohort="source_reported_only",
                                baseline=target + ":" + label + ":" + cutoff,
                                stage=stage,
                                unit=source_unit,
                                count_kind=kind,
                                count=sum(bool(d) and d <= cutoff for d in observed)
                                if supported
                                else "",
                                missing_count=sum(not d for d in observed)
                                if supported
                                else "",
                                lost_visibility_count="",
                                ...
                                evidence_ids="catalogue",
                                reason="no historical census/pending evidence; current start is not approval",
                            )
                        )
    return coverage


def build_document_coverage(selected, detailed, assessments):
    docs = []
    for row in selected:
        ...
        for stage in ("approval", "procurement", "start"):
            docs.append(
                dict(
                    ...
                    stage=stage,
                    structured_record_available=str(bool(detail)).lower(),
                    linked_documents=len(links) if detail else "",
                    local_stage_document="not_established",
                    attempted="false",
                    retrieved="false",
                    validates_stage="false",
                    observation="pointers_only"
                    if links
                    else "no_links_in_payload"
                    if detail
                    else "not_attempted",
    ...
    for row in docs:
        match = next(
            (
                a
                for a in assessments
                if a["case_id"] == row["case_id"] and a["stage"] == row["stage"]
            ),
            None,
        )
        if match:
...
def event(ident, stage, field, raw, date, precision, evidence, validation, missing):
    return dict(
        ...
        stage=stage,
        raw_field=field,
        raw_value=json.dumps(raw, sort_keys=True) if raw != "" else "",
        normalized_date=date,
        interval_start="",
        interval_end="",
        precision=precision,
        source_coverage="current snapshot; no independent prior coverage",
        source_version=SNAPSHOT,
        validation_status=validation,
        evidence_id=evidence,
        missingness_reason=missing,
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input", type=Path, required=True, help="0735-round3 archive directory"
    )
