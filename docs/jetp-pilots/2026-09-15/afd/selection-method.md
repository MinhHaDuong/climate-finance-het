...
Reserve follow-up, logged after units 1–24 and before unit 25: all historical
project links return HTTP→HTTPS redirects. Spend the six reserved units following
CMA123501, CTN129703 and CZZ276103 in that order, at most two requests per case.
These discriminate a broken transport route from a missing exact-ID document.
Stop a case on nonredirect or after two requests; no broader discovery. Keep the
remaining sample nonresponse, including two not-attempted cases, unchanged.

Units 25–27 each ended at HTTP 404, leaving three verification units. Before
unit 28, allocate these to one exact-financing-ID query each for CMA123501,
CTN129703 and CZZ276103, restricted to the AFD publisher. This tests whether a
