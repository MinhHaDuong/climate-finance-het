...
- **Design for the migrated tables, not the legacy ones the site still
  serves.** `events.csv` is on its way to `observations`, and
  `project-source-links.csv` to `refers_to` rows of `relations`, so a
  structure read off the legacy tables encodes a pipeline that no longer
  exists.
