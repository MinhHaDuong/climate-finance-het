...
abstract: |
  ...
  A two-pass censored-gap analysis tests whether validated zones survive
  the removal of transitional years. Applied to this corpus the rule returns
  a single zone covering the whole analysis window: at the thresholds used
  here it does not localise a break, and the three layers instead show a
  distribution in continuous motion. The framework does not claim to
...
Because the corpus grows over time, the after-window is typically larger than the before-window. All statistics are computed on equal-$n$ subsamples (§4.5), and the subsampling variance is shown explicitly as a ribbon in @fig-zseries.
...
*Permutation Z-scores for S2, L1, G9.* For each $(t, w)$, we pool the two windows, relabel them at random $B = 500$ times while preserving sample sizes, and recompute the statistic. The standardised effect size $Z(t) = (S_{\text{obs}} - \bar{S}_{\text{perm}}) / \mathrm{sd}(S_{\text{perm}})$ is directly comparable across years despite changing corpus size. A permutation scheme is appropriate here because the observed statistic has no closed-form null under general distributional alternatives. To expose subsampling variance in the numerator, we draw $R = 20$ independent equal-$n$ subsamples of the observed side and compute a trimmed Z-score ribbon: the central sixteen values (discarding the two extremes on each end, equivalent to the 10th and 90th percentiles) bound the shaded region in @fig-zseries. The denominator --- the permutation null mean and standard deviation --- is identical across all twenty draws; only the observed subsample changes.
...
@fig-zseries presents permutation Z-scores for the three distance detectors across the full corpus of {{< meta corpus_total >}} works. At the lead window half-width $w = 3$, S2 energy distance peaks at year {{< meta s2_peak_year_w3 >}}, L1 Jensen--Shannon divergence peaks at year {{< meta l1_peak_year_w3 >}}, and G9 community divergence peaks at year {{< meta g9_peak_year_w3 >}}. The three peaks fall in the last third of the analysis window, and they disagree with one another. They also move with the half-width: varying $w$ over $\{2, 3, 4\}$ (§4.8) displaces the S2, L1, and G9 peaks by {{< meta s2_peak_spread_w234 >}}, {{< meta l1_peak_spread_w234 >}}, and {{< meta g9_peak_spread_w234 >}} years respectively. A peak year is therefore a weak summary of these series, which is the reason the framework reports zones rather than point breakpoints (§4.3).

Applying the two-layer agreement rule yields one validated transition zone, running from {{< meta zone_1_start >}} to {{< meta zone_1_end >}}. Every year in that span satisfies the rule, so the supra-threshold run is uninterrupted and covers the analysis window; @fig-heatmap marks every column. At a threshold of $|Z| > 2$ with two agreeing layers, the rule does not discriminate within this corpus: it selects the whole period rather than isolating candidate breaks. What the three layers show is a distribution in continuous motion, not one that steps at a date.

The Z-scores also rise broadly with time on all three layers. One possible explanation is that detection power tracks the yearly document count, which grows across the window; separating that from a genuine acceleration would require a permutation scheme conditioning on year-specific sample sizes, which this design does not use.

Two downstream steps are therefore not reported. The censored-gap confirmation of §4.7 does not apply to a zone that spans the analysis window, since excising its years leaves no before-and-after to compare. The interpretive outputs of §4.9 are withheld for the same reason. The discriminative-term and community-shift figures require interpretation tables that have not been built (ticket 0064).

### 5.2 No break in the core subset

Restricting the corpus to its {{< meta corpus_core >}} most-cited works (citation count ≥ {{< meta corpus_core_threshold >}}) eliminates the validated zone: no method exceeds the $Z > 2$ threshold at any year under this filter.

The null result is substantively informative. The most-cited works in climate finance span the full analysis window and are not concentrated in any particular sub-period. The movement visible in the full corpus reflects changing *composition* --- new communities entering and new vocabulary becoming standard --- rather than a reorientation among the works that anchored the intellectual canon. The core subset remained stable throughout.

### 5.3 The efficiency--accountability axis

Projecting the corpus onto the efficiency--accountability axis yields a continuum, not two camps. A two-component Gaussian mixture fits the projected scores better than a single Gaussian, by the Bayesian Information Criterion: $\Delta BIC_{\text{emb}}$ = {{< meta bim_dbic_embedding >}} on the embedding layer and $\Delta BIC_{\text{lex}}$ = {{< meta bim_dbic_tfidf >}} on the TF-IDF lexical layer. That improvement measures departure from normality rather than separation. The components of the fitted mixture lie {{< meta bim_gmm_separation >}} standard deviations apart, where roughly two are needed for a mixture density to have two humps, and counting the modes of that fitted density returns {{< meta bim_gmm_modes >}}. Hartigan's dip test, whose null hypothesis is unimodality itself and which assumes no parametric family, does not reject: $p$ = {{< meta bim_dip_p_embedding >}}.

Nor does it reject in any sub-period: $p$ = {{< meta bim_dip_p_pre2007 >}} before 2007, {{< meta bim_dip_p_2007_2014 >}} across 2007--2014, and {{< meta bim_dip_p_post2015 >}} after 2015. The BIC differences move over the same span --- {{< meta bim_dbic_pre2007 >}} before 2007, {{< meta bim_dbic_2007_2014 >}} across 2007--2014, {{< meta bim_dbic_post2015 >}} after 2015 --- and the last is the largest. But they are not comparable across sub-corpora of {{< meta bim_n_pre2007 >}}, {{< meta bim_n_2007_2014 >}} and {{< meta bim_n_post2015 >}} works: the likelihood gain grows with sample size while the BIC penalty grows only with its logarithm, so a later period inherits a larger difference from its size alone.

The axis itself is real and externally valid. Its embedding and lexical constructions agree at $r$ = {{< meta bim_corr >}}, the keyword-assigned ends hold {{< meta bim_n_efficiency >}} efficiency-oriented and {{< meta bim_n_accountability >}} accountability-oriented works, and those ends concentrate in different journal ecologies. What the corpus does not show is a gap in the middle.

This is the expected shape if the poles are positions within a shared debate rather than separate literatures. Papers draw on both vocabularies in varying proportion; the ends of the axis are populated, and so is everything between them.
...
PC2 explains {{< meta pca_emb_pc2_var_pct >}}% of the variance and aligns with the efficiency--accountability axis: its cosine similarity with the seed vector is {{< meta pca_emb_pc2_cosine >}}, and fitting the two-cluster GMM on the PC2 projection alone yields a BIC difference of {{< meta pca_emb_pc2_dbic >}}. The axis identified in §5.3 therefore projects onto the second principal component. That BIC difference carries the same caveat as those in §5.3 and is not evidence of two modes.

An earlier draft of this analysis placed the structure on PC1; the corrected computation on the full corpus shows that PC1 captures temporal growth and PC2 captures the efficiency--accountability contrast.

![Permutation Z-scores at the lead window ($w = 3$) for the four permutation-tested methods (S2 Energy, L1 JS, G9 Community, G2 Spectral) plus the C2ST AUC references (embedding and lexical). The grey band marks $|Z| < 2$, the permutation-based null 95% reference; dashed horizontals mark the $Z = \pm 2$ thresholds. Shaded ribbons around the method curves span the trimmed subsampling range — the central sixteen of twenty equal-$n$ draws (§4.5). C2ST AUCs are plotted against the twin right-hand axis.](../_shared/figures/fig_companion_zseries.png){#fig-zseries}

![Transition-zone validation heatmap. Rows are the six detection methods; columns are years from 1998 to 2021. Cell colour encodes the signed signal — Z-score for the four distance detectors, and $(\mathrm{AUC} - 0.5) \times 20$ for the two C2ST rows so that all rows share a comparable scale. Vertical rectangles mark years where at least two rows exceed the threshold ($|Z| > 2$, or rescaled AUC $> 2$) simultaneously (validated zones).](../_shared/figures/fig_companion_heatmap.png){#fig-heatmap}

The discriminative-term and community-shift plots are withheld until the interpretation tables are computed (ticket 0064). No figure is shown for either result.
...
*Multilingual coverage.* LDA and STM require language-specific stopword lists and tokenisation. Multilingual sentence-transformers (here `BAAI/bge-m3`) are trained to place documents in a shared semantic space regardless of language, and the citation graph is language-agnostic by construction.

Topic models retain advantages in labeled interpretability: a topic defined by its top-$k$ words is immediately readable. The framework's L1 lexical layer can support a discriminative-term interpretation (§4.9), but that output is not yet available; it does not generate a full topic labelling.
...
The framework does not claim to recover a unique true periodization. It produces a reproducible ledger of validated zones annotated with the layers that support them. The planned discriminative-term, document, and community-shift annotations remain unavailable. Applied to climate finance scholarship (1998--2021, {{< meta corpus_total >}} works), it returns one zone spanning the analysis window. That is a null result for localisation rather than a periodization: at these thresholds the agreement rule selects every year, and the layers agree on continuous movement rather than on a date.
...
@fig-companion-sensitivity shows the S2 energy distance profile under a 4×4 parameter grid of window half-width $w \in \{1, 3, 5, 7\}$ and censored-gap $g \in \{0, 1, 3, 5\}$ years. Each panel shows five curves for PCA dimensionalities (64, 128, 256, 512, full). Centre lines are medians over $R=3$ independent equal-$n$ subsample draws. The sweep moves parameters on the embedding layer alone: within each cell the energy statistic is standardised across that cell's years, so a curve places years relative to one another rather than against the permutation null of §4.5, and no cell re-runs the two-layer agreement rule. The model choice (BAAI/bge-m3) is fixed; extending to alternative embeddings is left for future work.

The grid records parameter dependence rather than confirming a stable profile, because the quantity such a check would fix does not hold still. Section 5.1 measures the movement directly on the lead analysis: over the narrower range $w \in \{2, 3, 4\}$ the S2, L1, and G9 peak years already shift by {{< meta s2_peak_spread_w234 >}}, {{< meta l1_peak_spread_w234 >}}, and {{< meta g9_peak_spread_w234 >}} years, and the agreement rule returns a single zone covering {{< meta zone_1_start >}}--{{< meta zone_1_end >}}. Widening $w$ to 7 pools more years on each side of every candidate year, which by construction attenuates the year-to-year sampling variability that $w = 1$ exposes, together with any short-lived shock. No localised break survives for the grid to hold fixed, and that dependence on the parameters is the sensitivity finding: it is the same reason §5.1 reports a zone and continuous movement rather than a breakpoint year.

Two limits bound what this appendix supports. The gap and dimension axes are not anchored to a validated zone, so the sweep licenses no claim about their effect on localisation --- with no localised break in the results, they have nothing to displace. And the grid covers S2 only; the L1 and G9 layers that vote in the agreement rule are not swept here.

![Sensitivity sweep: 4×4 grid of window × gap, with up to 5 PCA dims per cell. Curves are S2 energy distance standardised across the years of each cell. Cells marked “No eligible years” have no observations meeting the window and gap constraints.](../_shared/figures/fig_companion_sensitivity.png){#fig-companion-sensitivity}
