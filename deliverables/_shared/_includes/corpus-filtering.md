...
The full filtering pass (`corpus_filter.py --apply`) runs after enrichment (§2) has populated abstracts, citations, and embeddings. It implements a four-phase pipeline applying six flags — three cheap flags that need no external data, and three that depend on enrichment outputs. Five flags can remove; flag 5 is diagnostic only.
...
5. **Semantic distance (diagnostic):** Cosine distance from each paper's embedding to the centroid of its own language, falling back to the corpus centroid below {{< meta filter_outlier_min_lang >}} works. It is published as `semantic_outlier_dist` and removes no work. Requires `embeddings.npz` from §2.4.
