...
Drafting aid for the voice-alignment pass (ticket 0243), not an artefact of
the production chain — do not cite, do not include in the manuscript. Colocated
with `manuscript.qmd` because every pair below is anchored to specific lines
in that file at the time of writing; delete once the pass is complete and the
manuscript is published, same discipline as `style-references/README.md`.
...
numbers refer to `manuscript.qmd` at commit time.
