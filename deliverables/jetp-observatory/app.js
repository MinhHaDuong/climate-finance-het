...
<div class="comparison-banner"><div><p class="eyebrow">A longer view</p><h2>What did ordinary energy<br>projects look like?</h2><p>Browse the historical pool by country, instrument and approval year. See the distribution behind a typical financing window, not just an average.</p></div><div><p class="note">Administrative closure is not a measure of physical completion. This first pool is descriptive; the matched lifecycle study is a separate research task.</p><a class="button" href="#comparison">Explore ${overview.historical_count} closed operations ↗</a></div></div>`;
}
...
function eventView(e, sources) {
  const timing = e.date
    ? date(e.date)
    : e.observed_date
      ? `Reported as of ${date(e.observed_date)}`
      : e.recorded_date
        ? `Register date ${date(e.recorded_date)}`
        : "Date not established";
  return `<li><div class="date">${esc(timing)} · ${esc(e.date_basis)}</div><h3>${esc(e.status)}${e.amount != null ? " · " + esc(money(e.amount, e.currency)) : ""}</h3>${e.funder ? `<p>${esc(e.funder)} · ${esc(e.instrument)}</p>` : ""}<p>${esc(e.notes)}</p>${sourceLink(sources[e.source_id], "Source")} <span class="date-tag">${esc(e.locator)}</span></li>`;
}
